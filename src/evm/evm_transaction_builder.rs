...
/// Builder for [`EVMTransaction`](super::evm_transaction::EVMTransaction) values with native EIP-1559 fields.
...
impl EVMTransactionBuilder {
    ...
    /// Sets the target chain ID (EIP-155 domain separator).
    pub const fn chain_id(mut self, chain_id: u64) -> Self {
        self.chain_id = Some(chain_id);
        self
    }

    /// Sets the sender nonce (transaction sequence number).
    pub const fn nonce(mut self, nonce: u64) -> Self {
        self.nonce = Some(nonce);
        self
    }

    /// Sets the recipient address; omit for contract creation.
    pub const fn to(mut self, to: Address) -> Self {
        self.to = Some(to);
        self
    }

    /// Sets the amount of native currency to transfer.
    pub const fn value(mut self, value: u128) -> Self {
        self.value = Some(value);
        self
    }

    /// Sets the calldata payload; pass an empty `Vec` for pure ETH transfers.
    pub fn input(mut self, input: Vec<u8>) -> Self {
        self.input = Some(input);
        self
    }

    /// Sets the execution gas limit.
    pub const fn gas_limit(mut self, gas_limit: u128) -> Self {
        self.gas_limit = Some(gas_limit);
        self
    }

    /// Sets the maximum fee per gas (base fee cap).
    pub const fn max_fee_per_gas(mut self, max_fee_per_gas: u128) -> Self {
        self.max_fee_per_gas = Some(max_fee_per_gas);
        self
    }

    /// Sets the maximum priority fee per gas (tip cap).
