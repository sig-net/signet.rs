...
use super::io::{BufRead, Error, Read};
...
pub trait Decodable: Sized {
    fn decode_from_finite_reader<R: BufRead + ?Sized>(reader: &mut R) -> Result<Self, Error> {
        Self::decode(reader)
    }

    fn decode<R: BufRead + ?Sized>(reader: &mut R) -> Result<Self, Error>;
}
